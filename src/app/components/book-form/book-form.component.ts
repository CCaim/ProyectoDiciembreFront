...
export class BookFormComponent implements OnInit {
  ...
  constructor(
    ...
    private _libService: LibroService
  ) {}
  ...
  ngOnInit(): void {
    ...
    this._usuService.getUsuarioByUsername(username!).subscribe(
      (respuesta: Usuario) => {
        ...
        console.log('Usuario asociado al libro:', this.libroNew.usuario);
      }
    );
  }
  ...
  onRemove(event: any): void {
    console.log('Archivo eliminado:', event);
    ...
    if (this.imagenes.length > 0) {
      ...
      this._cloudinaryService.cargarImagen(data)
        ...
        .catch(error => {
          ...
          alert('Hubo un error al subir la imagen. Inténtelo nuevamente.');
        });
    } else {
    ...
    if (form.valid && this.imagenSubida) {
      this._libService.createLibro(this.libroNew).subscribe(
        ...
        error => {
          ...
          alert('Hubo un error al guardar el libro. Inténtelo nuevamente.');
        }
      );
    } else {
      alert('Complete todos los campos obligatorios y suba una imagen antes de guardar.');
    }
  }
}